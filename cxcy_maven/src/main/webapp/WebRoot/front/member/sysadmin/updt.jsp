...
<%
	...
	String flag="";
	String yhbh="",yhm="",realname="",phone="",xb="",mm="",bmbh="";	
	int count = 0; 	
	
	yhbh = ((Map<String,String>)session.getAttribute("front_userinfo")).get("yhbh");
	...
	{		
		...
		boolean result = true;		
		if(!"".equals(mm)){
			String prepsql="update admin set yhm=?,realname=?,phone=?,xb=?,mm=?,bmbh=? where yhbh='"+yhbh+"'";		
			result = DataBase.prepare(datasource,prepsql,yhm,realname,phone,xb,mm,bmbh);					
		}else{
			String prepsql="update admin set yhm=?,realname=?,phone=?,xb=?,bmbh=? where yhbh='"+yhbh+"'";		
			result = DataBase.prepare(datasource,prepsql,yhm,realname,phone,xb,bmbh);					
		}
