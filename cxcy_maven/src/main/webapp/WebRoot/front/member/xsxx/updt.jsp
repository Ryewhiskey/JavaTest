...
<%
	...
	String flag="";
	String sid="",xh="",name="",xb="",zy="",bmbh="",phone="",mm="",email="";	
	int count = 0; 	
	
	sid = ((Map<String,String>)session.getAttribute("front_userinfo")).get("yhbh");
	flag = request.getParameter("flag");
	if( sid!=null && !"".equals(sid.trim()) ){
		String sql = "select xh,name,xb,zy,bmbh,phone,email,mm from student where sid = '" + sid + "'";
	...
	{		
		...
		if(!"".equals(mm)){		
			String prepsql="update student set xh=?,name=?,xb=?,zy=?,bmbh=?,phone=?,email=?,mm=? where sid='"+sid+"'";		
			result = DataBase.prepare(datasource,prepsql,xh,name,xb,zy,bmbh,phone,email,mm);					
		}else{
			String prepsql="update student set xh=?,name=?,xb=?,zy=?,bmbh=?,phone=?,email=? where sid='"+sid+"'";		
			result = DataBase.prepare(datasource,prepsql,xh,name,xb,zy,bmbh,phone,email);					
		}
		if(result==false) {
%>
			<script type="text/javascript">			
				alert("有关联数据不能修改，请检查数据？");						
			</script>
