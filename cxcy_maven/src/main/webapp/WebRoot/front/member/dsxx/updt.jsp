...
<%	
	String flag="";
	String tid="",jsbh="",name="",zy="",phone="",xb="",mm="",zc="",bmbh="",yjly="",email="";
	int count = 0; 	
	
	tid = ((Map<String,String>)session.getAttribute("front_userinfo")).get("yhbh");
	...
	{		
		...
		if(!"".equals(mm)){		
			String prepsql="update teacher set jsbh=?,name=?,xb=?,zy=?,bmbh=?,zc=?,yjly=?,phone=?,email=?,mm=? where tid='"+tid+"'";		
			result = DataBase.prepare(datasource,prepsql,jsbh,name,xb,zy,bmbh,zc,yjly,phone,email,mm);					
		}else{
			String prepsql="update teacher set jsbh=?,name=?,xb=?,zy=?,bmbh=?,zc=?,yjly=?,phone=?,email=? where tid='"+tid+"'";		
			result = DataBase.prepare(datasource,prepsql,jsbh,name,xb,zy,bmbh,zc,yjly,phone,email);					
		}
		if(result==false) {
%>
			<script type="text/javascript">			
...
<div class="box-positon">
	<div class="rpos">当前位置: 管理中心 - 个人信息</div>
...
<%} %>			
		...
		<tr>
			...
			<td colspan="1" width="38%" class="pn-fcontent">
				<input type="text" name="yjly" value="<%=yjly %>" style="width:99%;" /><br/>
				多个研究领域之间用,隔开
			</td>
		</tr>		
