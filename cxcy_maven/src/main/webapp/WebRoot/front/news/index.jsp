...
<% 
	...
	totalRecords = DataBase.recordCount(datasource, "select id,title,fbrq from news where issuccess='2'");		
	...
	notes = null;	
	if( totalPages>0 ){	
		notes = DataBase.listQuery(datasource,"select id,title,fbrq from news where issuccess='2' order by fbrq desc",pageNo,PAGE_SIZE);	
...
%>
        <div class="right" >
        	<div class="content_search_result" title="信息列表内容展示" >
                ...
                <div id="search_result" title="信息列表内容" >
...
<%} %>                               
                        ...
                        <div id="showpages" style="text-align:center;">
	                        <form name="form1" method="post" target="_self">
	                    		共 <%=totalRecords %> 条&nbsp;
								每页 <%=PAGE_SIZE %> 条&nbsp;
								<input class="first-page" type="button" value="首 页" onclick="this.form.action='front/news/index.jsp?pageNo=1';this.form.submit();" <% if(pageNo<=1) out.print("disabled='disabled'"); %>/>
								<input class="pre-page" type="button" value="上一页" onclick="this.form.action='front/news/index.jsp?pageNo=<%=pageNo-1 %>';this.form.submit();" <% if(pageNo<=1) out.print("disabled='disabled'"); %>/>
								<input class="next-page" type="button" value="下一页" onclick="this.form.action='front/news/index.jsp?pageNo=<%=pageNo+1 %>';this.form.submit();" <% if(pageNo>=totalPages) out.print("disabled='disabled'"); %>/>
								<input class="last-page" type="button" value="尾 页" onclick="this.form.action='front/news/index.jsp?pageNo=<%=totalPages %>';this.form.submit();" <% if(pageNo>=totalPages) out.print("disabled='disabled'"); %>/>&nbsp;
								当前 <%=pageNo %>/<%=totalPages %> 页 &nbsp;转到第<input type="text" name="gopageno" id="_goPs" value="<%=pageNo %>" style="width:50px" onkeypress="if(event.keyCode==13){this.form.action='front/news/index.jsp?pageNo='+this.value;this.form.submit();}"/>页
								<input class="go" name="_goPage" type="button" value="转" onclick="this.form.action='front/news/index.jsp?pageNo='+this.form._goPs.value;this.form.submit();"/>
	                    	</form>	
                    	</div>
               			</div>
                    </div>
