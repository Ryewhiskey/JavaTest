...
<%
	...
	if( !RWfile.deleteFile(getServletContext().getRealPath("/UploadDir/download")+"/"+filename) ){
%>
...
<%		
				...
				DataBase.executeUpdate(datasource,"delete from zlxzb where id = '" + id + "'");
			}
		}		
...
<div class="box-positon">
	<div class="rpos">当前位置: 内容 - 下载专区</div>
	<form class="ropt">
...
<form name="form1" action="admin.jsp" method="post" style="padding-top:5px;padding-bottom:5px;">
	...
	发布人: <input type="text" name="query2" value="<%=query2 %>" style="width:150px"/>
	发布日期: <input type="text" name="query3" value="<%=query3 %>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate"/>
	审核: <select name="query4">
			<option value="0" <% if(query4!=null && "0".equals(query4)) out.print("selected");%>>全部</option>
			<option value="1" <% if(query4!=null && "1".equals(query4)) out.print("selected");%>>未审核</option>
			<option value="2" <% if(query4!=null && "2".equals(query4)) out.print("selected");%>>审核</option>				
		</select>
...
%>
  <tr onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'"> 
    ...
    <td class="style3" ><div align="center">
		...
		||&nbsp;&nbsp;<a href="admin.jsp?id=<%=note[0].toString() %>&act=d&filename=<%=note[4].toString() %>" onclick="return confirm('删除操作无法恢复！是否继续？')" >删除</a>
