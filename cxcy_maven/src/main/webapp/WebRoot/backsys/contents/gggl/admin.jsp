...
<%
	...
	if(flag == null || flag.equals("form1") ){	
		...
		sql = "select id,title,dw,fbrq,content,issuccess from ggb where 1=1 ";						
		if( query1 !=null && !"".equals(query1) ){
			sql += " and " + "title like '%"+query1+"%' ";
		}
		if( query2 !=null && !"".equals(query2) ){
			sql += " and " + "dw like '%"+query2+"%' ";
		}
		if( query3 !=null && !"".equals(query3) ){
			sql += " and " + "fbrq like '%"+query3+"%' ";
		}
		if( query4 !=null && !"".equals(query4) ){
			sql += " and " + "content like '%"+query4+"%' ";
		}
		if( query5 !=null && !"".equals(query5) && !"0".equals(query5)){
			sql += " and " + "issuccess like '%"+query5+"%' ";
		}
	...
	{	String id="";	
		...
		delcount=DataBase.executeUpdate(datasource,"delete from ggb where id = '" + id + "'");
		if(delcount==0){
%>
...
<div class="box-positon">
	<div class="rpos">当前位置: 内容 - 公告管理</div>
	<form class="ropt">
...
<form name="form1" action="admin.jsp" method="post" style="padding-top:5px;padding-bottom:5px;">
	...
	标题: <input type="text" name="query1" value="<%=query1 %>" style="width:100px"/>
	单位: <input type="text" name="query2" value="<%=query2 %>" style="width:150px"/>
	发布日期: <input type="text" name="query3" value="<%=query3 %>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate"/>
	内容: <input type="text" name="query4" value="<%=query4 %>" style="width:150px"/>
	审核: <select name="query5">
			<option value="0" <% if(query5!=null && "0".equals(query5)) out.print("selected");%>>全部</option>
			<option value="1" <% if(query5!=null && "1".equals(query5)) out.print("selected");%>>未审核</option>
			<option value="2" <% if(query5!=null && "2".equals(query5)) out.print("selected");%>>审核</option>				
		</select>
...
<%
	if(sql!=null && !"".equals(sql) ){	
		...
		if( totalPages>0 ){
			...
			notes = DataBase.listQuery(datasource,sql+" order by fbrq desc",pageNo,PAGE_SIZE);	
		...
		for (Iterator<Object[]> it=notes.iterator();it.hasNext();i++) {	
			Object[] note = it.next();	
			if("1".equals(note[5].toString()))	note[5]="未审核";
			if("2".equals(note[5].toString()))	note[5]="审核";											
%>
