...
<%
	...
	if(flag == null || flag.equals("form1") ){	
		...
		sql = "select lxnf,xmjb,xmbh,xmmc,xmlx,student.name as sname,xh,student.phone as sphone,cyrs,qtcy,dsbh,dsbh,yjxkdm,zzje,jtsj,xmjj,qtds,xmhzb.id,xkdmb.name as xkmc from xmhzb,student,xkdmb where xmhzb.fzr=student.xh and xmhzb.yjxkdm=xkdmb.id "; 
...
<div class="box-positon">
	<div class="rpos">当前位置: 项目 - 项目管理</div>
	<form class="ropt">
		<input class="add" type="submit" value="添加" onclick="this.form.action='add.jsp';"/>
		<input class="add4" type="submit" value="导出数据" onclick="this.form.action='../expdata/excel_dc.jsp';"/>
	</form>
	<div class="clear"></div>
</div>
<div class="body-box">
<form name="form1" action="admin.jsp" method="post" style="padding-top:5px;padding-bottom:5px;">
	...
	负责人: <input type="text" name="query5" value="<%=query5 %>" style="width:70px"/>
...
<%
	if(sql!=null && !"".equals(sql) ){	
		...
		if( totalPages>0 ){
			...
			notes = DataBase.listQuery(datasource,sql+" order by lxnf desc,right(xmbh,3),sname",pageNo,PAGE_SIZE);	
		...
		for (Iterator<Object[]> it=notes.iterator();it.hasNext();i++) {	
			Object[] note = it.next();			
			if(note[2]==null) note[2]="111111111111";										
%>
  <tr onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'"> 
    ...
    <td class="style3" ><div align="center"><% if(note[18]!=null) out.print(note[18].toString());else out.print("&nbsp;"); %></div></td>
        
    <td class="style3" ><div align="center">
		<a href="updt.jsp?id=<%=note[17].toString() %> ">修改</a>
    	|| <a href="admin.jsp?id=<%=note[17].toString()%>&act=d" onclick="return confirm('删除操作无法恢复！是否继续？')" >删除</a>
